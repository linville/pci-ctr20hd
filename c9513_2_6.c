...
static int ctr20HD_ioctl(struct inode *iNode, struct file *filePtr, unsigned int cmd, unsigned long arg);
...
static struct pci_device_id ctr20HD_id_tbl[] __devinitdata = {
  {
...
static int __devinit ctr20HD_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
  int i,j;
  int minor;
  u16 base1;
  ...
  for (i = 1; i <= NCOUNTERS; i++) {
    for (j = 0; j < NCHIP; j++) {
      ...
      sprintf(name, "ctr%d_%d%d", NumBoards, j, i);
      device_create(ctr20HD_class, NULL, MKDEV(MajorNumber, minor), name);
    }
  }
  
  printk("%s: address=%#x IRQ=%d.", ADAPTER_ID, BoardData[NumBoards].base2, pdev->irq);
  printk(" 10/14/2008 wjasper@tx.ncsu.edu\n");
